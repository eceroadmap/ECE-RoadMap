  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-5 rounded-3xl bg-[#091527] border border-slate-800 shadow-xl">
        ...
        <button
          onClick={async () => {
            if (window.confirm('هل ترغب في حذف وتطهير كل النصائح المولدة تلقائياً / الوهمية من قاعدة البيانات السحابية؟')) {
              setIsLoading(true);
              try {
                const count = await firebaseSyncService.purgeAutoGeneratedTips();
                await loadTips();
                alert(`تم تطهير وحذف ${count} نصيحة مولدة تلقائياً بنجاح! ولم يتبق سوى مشاركات الطلاب الحقيقية.`);
              } catch (e) {
                console.error('Purge failed:', e);
                alert('حدث خطأ أثناء تطهير النصائح.');
              } finally {
                setIsLoading(false);
              }
            }
        ...
        >
          ...
          <span>تطهير النصائح المولدة تلقائياً</span>
        </button>
      </div>
