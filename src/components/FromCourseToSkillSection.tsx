...
interface SkillPipeline {
  id: string;
  category: 'signals' | 'embedded' | 'networks' | 'rf' | 'circuits';
...
const SKILL_PIPELINES: SkillPipeline[] = [
  {
    id: 'dsp-pipeline',
    category: 'signals',
...
export const FromCourseToSkillSection: React.FC<FromCourseToSkillSectionProps> = ({
  ...
  const [activePipelineId, setActivePipelineId] = useState<string>(SKILL_PIPELINES[0].id);

  const selectedPipeline = SKILL_PIPELINES.find((p) => p.id === activePipelineId) || SKILL_PIPELINES[0];
