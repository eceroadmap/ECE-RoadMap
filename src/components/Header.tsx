...
export const Header: React.FC<HeaderProps> = ({
  ...
  // Primary navigation items (always shown on Desktop and Laptop)
  const primaryNavItems: NavItem[] = [
    { id: 'home', label: 'الرئيسية', icon: Compass },
    { id: 'dashboard', label: 'رحلتي', icon: GraduationCap, badge: profile.currentYear ? (profile.currentYear === 'graduate' ? 'خريج' : `سنة ${profile.currentYear}`) : undefined },
  ...
  // Secondary navigation items (shown directly on Desktop >=1280px, inside "More" on Laptop 1024-1279px)
  const secondaryNavItems: NavItem[] = [
    { id: 'academic_record', label: 'سجلي الأكاديمي', icon: Award },
    { id: 'graduation_projects', label: 'مستكشف مشاريع التخرج', icon: FolderGit2 },
    ...
    <header className="sticky top-0 z-40 w-full border-b border-cyan-950/60 bg-[#050b14]/95 backdrop-blur-md">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-2">
          ...
          {/* Desktop Navigation (>= 1280px): Show All items */}
            ...
            {allNavItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all duration-200 whitespace-nowrap ${
                    isActive
          ...
          {/* Laptop Navigation (1024px - 1279px): Primary Items + "More" Dropdown */}
          <nav className="hidden lg:flex xl:hidden items-center gap-1">
            {primaryNavItems.map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  id={`nav-link-laptop-${item.id}`}
                  onClick={() => handleNavClick(item.id)}
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition-all whitespace-nowrap ${
                    isActive
            ...
            {/* "المزيد" (More) Dropdown for Laptop */}
            <div className="relative" ref={moreDropdownRef}>
              ...
              {moreDropdownOpen && (
                <div className="absolute left-0 mt-2 w-48 rounded-xl bg-[#081528] border border-cyan-900/70 shadow-2xl p-1.5 z-50 animate-fadeIn">
                  {secondaryNavItems.map((item) => {
                    ...
                    return (
                      <button
                        key={item.id}
                        onClick={() => handleNavClick(item.id)}
                        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold text-right transition-colors ${
                          isActive
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 text-xs text-slate-300 bg-slate-900/80 hover:bg-slate-800 border border-slate-800 hover:border-cyan-500/50 rounded-xl transition-all group min-h-[44px]"
              aria-label="بحث شامل في المقررات والبرامج"
            >
              <Search className="w-4 h-4 text-cyan-400 group-hover:scale-110 transition-transform" />
              <span className="hidden lg:inline text-slate-300 font-medium">بحث في المواد والبرامج...</span>
              <kbd className="hidden xl:inline-block px-1.5 py-0.5 text-[10px] font-mono text-slate-400 bg-slate-800 border border-slate-700 rounded">
                Ctrl K
              </kbd>
            </button>
            ...
            <button
              ...
              className="lg:hidden p-2.5 rounded-xl text-slate-300 hover:text-white bg-slate-900/90 border border-slate-800 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 min-h-[44px] min-w-[44px] flex items-center justify-center"
      ...
      {/* Tablet & Mobile Slide-Down Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-800 bg-[#071120] px-4 pt-3 pb-6 shadow-2xl space-y-2 animate-fadeIn max-h-[85vh] overflow-y-auto">
