...
class FirebaseSyncService {
  ...
  /**
   * Helper function to identify auto-generated / dummy seeded tips
   */
  public isAutoGeneratedTip(tip: Partial<CommunityTip> & { id?: string }): boolean {
    const id = tip.id || '';
    const authorId = tip.authorId || '';
    const authorName = tip.authorName || '';
    return (
      id.startsWith('tip-official-') ||
      id.startsWith('tip-seed-') ||
  ...
  /**
   * Purges all auto-generated, mock, or hardcoded seed tips from Firestore & localStorage
   */
  public async purgeAutoGeneratedTips(): Promise<number> {
    let purgedCount = 0;
    try {
      try {
      ...
      for (const docSnap of snap.docs) {
        const data = docSnap.data() as CommunityTip;
        const tipWithId = { ...data, id: docSnap.id };

        if (this.isAutoGeneratedTip(tipWithId)) {
          try {
            await deleteDoc(doc(db, 'communityTips', docSnap.id));
            purgedCount++;
          } catch (delErr) {
            console.warn('Could not delete auto-generated tip:', docSnap.id, delErr);
          }
        }
      }
    } catch (err) {
      console.warn('Error purging auto-generated tips:', err);
    }
    return purgedCount;
  }

  public async rebuildCommunityTipsCollection(): Promise<void> {
    // Purge any lingering auto-generated tips without injecting any new dummy data
    await this.purgeAutoGeneratedTips();
  }

  public subscribeCommunityTips(callback: (state: TipsStateCallback) => void) {
    // Run background purge to clean any existing auto-generated tips
    this.purgeAutoGeneratedTips().catch(() => {});
    ...
    try {
      ...
      return onSnapshot(q, (snapshot) => {
        const firestoreTips: CommunityTip[] = snapshot.docs
          ...
          .filter(t => (t.status || 'active') === 'active' && !this.isAutoGeneratedTip(t));
